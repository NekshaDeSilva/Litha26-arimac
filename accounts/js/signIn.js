      ...
      snapshot.forEach(childSnapshot => {
        ...
        function gh(){
                 ...
                 if(userData.accountStatus ==1){
                    $('.userId_preview_descorp--dd-d').append(`<img data-inlinecss="yes" title="Verified Account" src="./assets/static/patchverified.png" style="width: 1.5rem; "    class="accountNVerified_badge" alt="Untititled" draggable="false" loading="lazy" onerror="this.src='./assets/static/patch_verfied.png'">`)
                }
        ...
        encryptDefKey();
            ...
            $('.ncloud-rolechange').html('<div class="errorHappened_div_ncloud-signin-Comp-inner"><img src="./assets//static/exclamation-diamond.svg" alt="error" draggable="false" loading="lazy" style="width: 5rem; margin-bottom: 1rem;"><span>Login failed.<br>' + String(window.__LITHA_DB_LAST_ERROR || 'Please check email or password.') + '</span><div onclick="window.location.href = \"\"">Retry</div></div>');
        }
      });
    } else {
        $('.ncloud-rolechange').html('<div class="errorHappened_div_ncloud-signin-Comp-inner"><img src="./assets//static/exclamation-diamond.svg" alt="error" draggable="false" loading="lazy" style="width: 5rem; margin-bottom: 1rem;"><span>Login failed.<br>' + String(window.__LITHA_DB_LAST_ERROR || 'Please check email or password.') + '</span><div onclick="window.location.href = \"\"">Retry</div></div>');
...
const password = String(userData.passW || '');
...
validateLogin(email, password);
    ...
    function checkSignIn(){
        ...
        database.ref().orderByChild('userData/Email').equalTo(userData.Email).once('value', function(existingSnapshot){
            if (existingSnapshot && existingSnapshot.exists()) {
                var matched = false;
                existingSnapshot.forEach(function(childSnapshot){
                    var existing = (childSnapshot.val() || {}).userData || {};
                    if (String(existing.passW || '') === String(userData.passW || '')) {
                        userData = existing;
                        matched = true;
                    }
                });
            ...
            database.ref(userData.userKeyId).set({ userData }).then(function(){
...
async function showOpenImage(){
    ...
    if (window.showOpenFilePicker) {
        try {
            const pickerOpts = {
                types: [{ description: "Profile Image", accept: { "image/*": [".png", ".gif", ".jpeg", ".jpg", ".webp"] } }],
    ...
    fallback.addEventListener('change', function(ev){
        applyFile(ev.target.files && ev.target.files[0]);
        setTimeout(function(){ fallback.remove(); }, 0);
    });
    document.body.appendChild(fallback);
    fallback.click();
}
function logInCheck_api_check(){
    var email = String($('.Email_accuireF_inputNormalpreset').val() || '').trim();
    var password = String($('.PassW_accuireF_inputNormalpreset').val() || '');
    if(!email || !password){
        $('.ncloud-rolechange').html('<div class="errorHappened_div_ncloud-signin-Comp-inner"><img src="./assets//static/exclamation-diamond.svg" alt="error" draggable="false" loading="lazy" style="width: 5rem; margin-bottom: 1rem;"><span>Login failed.<br>' + String(window.__LITHA_DB_LAST_ERROR || 'Please check email or password.') + '</span><div onclick="window.location.href = \"\"">Retry</div></div>');
        return;
    }

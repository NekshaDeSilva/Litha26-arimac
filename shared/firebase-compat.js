(function () {
  ...
  window.__LITHA_SUPABASE_CONFIG = {
    url: SUPABASE_URL,
  ...
  function setLastError(err) {
    try {
      window.__LITHA_DB_LAST_ERROR = err ? String(err.message || err) : '';
    } catch (_) {}
  }
  ...
  function makeSnapshotFromPairs(pairs) {
    ...
    return {
      ...
      val: function () {
        return arr.length === 0 ? null : (arr.length === 1
          ? arr[0]
          : Object.fromEntries((pairs || []).map(function (p) { return [p.key, p.value]; })));
      },
      forEach: function (cb) {
  ...
  async function selectPostsCompat(client, postId) {
    if (postId) {
      ...
      if (!fallbackA.error) {
        return fallbackA.data ? [{ id: fallbackA.data.id, postdata_: normalizePostData(fallbackA.data) }] : [];
      }
    ...
    if (!fallbackA.error) {
      return (fallbackA.data || []).map(function (row) {
        return { id: row.id, postdata_: normalizePostData(row) };
      });
    }
  ...
  Ref.prototype.orderByChild = function (path) {
    this._orderBy = { type: 'child', path: path };
    return this;
  };

  Ref.prototype.orderByKey = function () {
    this._orderBy = { type: 'key' };
    return this;
  };

  Ref.prototype.equalTo = function (value) {
    this._equalTo = value;
    return this;
  };
  ...
  Ref.prototype.transaction = function (updater, completion) {
    ...
    (async function () {
      try {
        if (!/^\/posts\/[^/]+\/postdata_\/likeCount$/.test(self.path)) {
          throw new Error('Unsupported transaction path ' + self.path);
        }
        ...
        const currentCount = Number((currentRow.postdata_ || {}).likeCount || 0);
        const desired = updater(currentCount);
        ...
        const { data: newLikeCount, error: rpcError } = await client.rpc('increment_litha_post_like', { post_id: postId });
        ...
        setLastError('');
        completion && completion(null, true, { val: function () { return Number(newLikeCount || desired || 0); } });
      } catch (err) {
        setLastError(err);
